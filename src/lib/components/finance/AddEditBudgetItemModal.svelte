<script lang="ts">
  ...
  let budgetItem: Omit<BudgetItem, 'id'> = currentBudgetItem ? { ...currentBudgetItem } : { name: '', budgeted: 0, totalSpent: 0, description: '' };
  let isLoading = false;
  ...
  <div class="bg-white text-black p-6 rounded-lg shadow-lg w-full max-w-md">
    ...
    <form on:submit|preventDefault={handleSubmit} class="space-y-4">
      ...
      <label class="block">
        ...
        <input type="number" bind:value={budgetItem.budgeted} placeholder="Amount" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" />
      </label>
      <label class="block">
        <span class="text-gray-700">Category</span>
        <div class="relative mt-2">
          <input id="combobox" type="text" bind:value={budgetItem.description} class="w-full rounded-md border-0 bg-white py-1.5 pl-3 pr-12 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" role="combobox" aria-controls="options" aria-expanded="false">
